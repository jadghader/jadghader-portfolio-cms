...
const Login: React.FC = () => {
  ...
  const [isEmailLoading, setIsEmailLoading] = useState<boolean>(false); // Separate loading state for email login
  ...
  useEffect(() => {
    // Redirect if logged in and isAdmin
    if (user && isAdmin) {
      window.location.href = "/admin/settings";
    }

    // If logged in but not admin, log them out and show error message
    if (user && !isAdmin) {
